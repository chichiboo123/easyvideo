...
- 재생헤드·클립 경계·마커 **스냅(자석)** — 자막/스티커/이미지/오디오 드래그에도 적용
...
- 자막·스티커·이미지를 **캔버스로 렌더링해 PNG 오버레이로 합성**
...
- @ffmpeg/ffmpeg (ffmpeg.wasm) — 클라이언트 영상 처리 (xfade·acrossfade·overlay)
- Canvas API — 자막/스티커/이미지 오버레이 렌더링 (내보내기 충실도)
...
│   ├── ProPreviewPanel.tsx      # 중앙 미리보기 (오디오 동기화 + 전환 프리뷰)
│   ├── PropertiesPanel.tsx      # 우측 속성 패널 (자막 프리셋 + 구간별 전환)
│   ├── ProTimeline.tsx          # 멀티트랙 타임라인 + 전환 배지 + 스냅
...
├── lib/
│   ├── ffmpeg.ts                # FFmpeg.wasm 래퍼 (xfade/acrossfade/overlay/adelay)
│   ├── overlayRender.ts         # 자막·스티커·이미지 → PNG 캔버스 렌더러
